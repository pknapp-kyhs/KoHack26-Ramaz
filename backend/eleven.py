...
def get_nested_text_segments(payload):
    if isinstance(payload, dict):
        for key in ("text", "he", "hebrew", "en", "english"):
            value = payload.get(key)
            if isinstance(value, (list, str, dict)):
                segments = flatten_text_segments(value)
                if segments:
                    return segments
    ...
    if isinstance(payload, (list, str)):
        return flatten_text_segments(payload)
...
def get_text_segments_from_versions(versions, lang):
    for version in versions:
        version_lang = (version.get("language") or "").lower()
        if version_lang == lang.lower():
            segments = get_nested_text_segments(version.get("text"))
            if segments:
                return segments
    return []
...
def get_daily_mishnah_data():
    ...
    hebrew_segments = (
        get_text_segments_from_versions(versions, "he")
        or get_text_segments_from_versions(versions, "hebrew")
        or get_nested_text_segments(data.get("he"))
